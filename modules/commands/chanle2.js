...
module.exports.config = {
	...
	try {
		...
		switch (r[0]) {
			...
			case "-c":
				...
				if (0 == await a.getData(i)) return e.sendMessage("Bạn không có đủ 50$ để tạo bàn game mới!!", r[1], s, o);
			...
			case "-j":
				...
				if (0 == await a.getData(i)) return e.sendMessage(`Bạn không có đủ $ để tham gia bàn game này! ${g.money}$`, s, o);
	...
	const {
		...
		body: r,
		senderID: g
	} = n, h = ["chẵn", "lẻ"], o = h[Math.floor(Math.random() * h.length)];
			...
			e.sendMessage({
				...
				setTimeout((async function() {
					...
					var t = o,
						r = [],
						h = [];
					if ("chẵn" == t) var i = ["https://i.imgur.com/6fIJU1q.jpg", "https://i.imgur.com/XPg6Uvq.jpg", "https://i.imgur.com/IWjB9kN.jpg", "https://i.imgur.com/XVxgPhY.png", "https://i.imgur.com/dRzktqf.png"];
					else if ("lẻ" == t) i = ["https://i.imgur.com/u1DjwX0.png", "https://i.imgur.com/unnBcv9.png", "https://i.imgur.com/181R8Te.jpg", "https://i.imgur.com/y67IGtv.jpg", "https://i.imgur.com/y67IGtv.jpg"];
					if (0 == t.indexOf("chẵn"))
					...
					var u = "KẾT QUẢ: " + t.toUpperCase() + "\n\nThua:\n",
						...
						p = 0;
					for (var d of r) {
						(b = await a.getData(d.userID)).money = b.money + parseInt(3 * n.money), await a.setData(d.userID, {
							data: b
						}), u += ++l + ". " + d.name + "\n"
					}
					for (var y of h) {
						var b;
						(b = await a.getData(y.userID)).money = b.money - parseInt(n.money), await a.setData(y.userID, {
							data: b
						}), 0 == p && (u += "\nThắng:\n"), u += ++p + ". " + y.name + "\n"
					}
